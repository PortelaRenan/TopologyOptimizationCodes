...
while conv == 0 
    ...
    remove = eig_aux(:,1) > 0;
    eig_aux = eig_aux(remove,:);
    omega = sqrt(eig_aux(1:3,1))/(2*pi);
...
for i=1:nel
    ...
    rectangle('Position',[0+step -d(i) Lx/nel 2*d(i)],'FaceColor',color1)
    step=step+Lx/nel;
end
...
xlabel('Iterations') 
ylabel('Frequency (Hz)')
legend({'1st Frequency','2nd Frequency','3rd Frequency'})
