...
nx = 241; % nodes number in x
ny = 41; % nodes number in y
Lx = 240/1000; % length in x [mm]
Ly = 40/1000; % length in y [mm]
...
%% material matrix
            %E    rho  nu
material = [200e9 7800 0.3];  % [Pa] [kg/m�] []
...
historical = zeros(nel,60); % history of the sensivity number vector
...
alpha_mean(:,1) = 1:nel;
alpha_mean(:,2) = zeros(nel,1);

tic

while conv == 0
    
    formatSpec = 'Iteração: %d\n';
...
%     alpha_mean(:,2) = mean(historical(:,1:iter),2);
    ...
    alpha_mean(:,2) = (alpha_i + alpha_mean(:,2))/2;
