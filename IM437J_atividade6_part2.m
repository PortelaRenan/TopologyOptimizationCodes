...
nx = 281; % nodes number in x
ny = 41; % nodes number in y
Lx = 14/100; % length in x [m]
Ly = 2/100; % length in y [m]
Lz = 1/100; % length in z [m]
...
for i = 1 : ny
    bc(2*i-1,:) = [1+nx*(i-1), 1, 0];
    bc(2*i,:) = [1+nx*(i-1), 2, 0];
end

for i = 1 : ny
    bc(2*i-1 + 2*ny,:) = [i*nx, 1, 0];
    bc(2*i + 2*ny,:) = [i*nx, 2, 0];
end

%% material matrix
            %E    rho  nu
material = [1e6 1 0.3];  % [Pa] [kg/m2] []
...
%Ar_max = 5/100;
r_min = 0.3/100; % maximal distance
tau = 0.01/100;
...
loc = [2*mass(1)-1 2*mass(1)];
mg(loc(1),loc(1)) = mg(loc(1),loc(1)) + mass(2); % global stiffness matrix assemble
mg(loc(2),loc(2)) = mg(loc(2),loc(2)) + mass(2); % global stiffness matrix assemble
